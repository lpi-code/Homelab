...
FIRST_BOOT_SCRIPT=""     # path to first-boot script (optional)
...
Usage:
  ./nanokvm-unattended.sh [--distro debian|proxmox] [--out ISO]
     ...
     [--ssh-pubkey PATH] [--root-pass PASS] [--answer-file PATH] [--first-boot PATH]
...
build_proxmox_unattended(){
  ...
  # Build prepare-iso command with optional first-boot script
  local prepare_cmd="prepare-iso $(basename "$in_iso") --fetch-from iso --answer-file answer.toml"
  if [ -n "$FIRST_BOOT_SCRIPT" ] && [ -f "$FIRST_BOOT_SCRIPT" ]; then
    prepare_cmd="$prepare_cmd --on-first-boot $(basename "$FIRST_BOOT_SCRIPT")"
    # Copy first-boot script to workdir
    cp "$FIRST_BOOT_SCRIPT" "${WORKDIR}/$(basename "$FIRST_BOOT_SCRIPT")"
    chmod +x "${WORKDIR}/$(basename "$FIRST_BOOT_SCRIPT")"
    note "📋 First-boot script: $(basename "$FIRST_BOOT_SCRIPT")"
  fi
  prepare_cmd="$prepare_cmd --output $(basename "$out")"
  note "📋 Command: $prepare_cmd"
  
  if run_proxmox_assistant $prepare_cmd; then
    # Move the output file to the expected location
    if [ -f "${WORKDIR}/$(basename "$out")" ]; then
      mv "${WORKDIR}/$(basename "$out")" "$out"
