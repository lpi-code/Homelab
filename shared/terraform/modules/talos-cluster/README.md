...
This Terraform module creates a complete Talos Kubernetes cluster on Proxmox, including control plane nodes, worker nodes, network infrastructure, and optional load balancing.
...
- **Optional load balancer** with HAProxy and Keepalived
- **Packer template integration** for consistent VM images
...
├─────────────────────────────────────────────────────────────┤
│  ┌─────────────────┐  ┌─────────────────┐  ┌──────────────┐ │
│  │   Load Balancer │  │  Control Plane  │  │   Workers    │ │
│  │   (Optional)    │  │     Nodes       │  │              │ │
│  │                 │  │                 │  │              │ │
│  │ HAProxy +       │  │ talos-cp-1      │  │ talos-w-1    │ │
│  │ Keepalived      │  │ talos-cp-2      │  │ talos-w-2    │ │
...
### Minimal Example (No Load Balancer)
...
| template_vm_id | Template VM ID to clone from (Packer-built) | `number` | n/a | yes |
...
1. **Packer-built Talos template** - Use the Packer configuration in the `talos-vm/packer/` directory
2. **Proxmox API access** - Configured with appropriate permissions
3. **Network planning** - IP ranges and VM IDs planned in advance
4. **SSH keys** - For load balancer access (if enabled)

## Deployment Process

1. **Build template** using Packer
...
- **Load balancer module** - For advanced load balancing

## Notes

- The cluster is automatically bootstrapped after all nodes are created
- Load balancer provides high availability for cluster endpoints
