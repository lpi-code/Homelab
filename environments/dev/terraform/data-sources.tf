...
locals {
  ...
  # Computed values for missing variables
  environment = "dev"

  iso_pool = data.external.ansible_vars.result.iso_pool
  openwrt_template_file_id = data.external.ansible_vars.result.openwrt_template_file_id
  talos_image_file_id = data.external.ansible_vars.result.talos_image_file_id
